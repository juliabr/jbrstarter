...
function breadcrumb_trail( $args = array() ) {
	...
	if ( is_singular() ) {
		...
		$defaults["singular_{$post->post_type}_taxonomy"] = 'recipe-category';
	}
	...
	if ( !empty( $trail ) && is_array( $trail ) ) {
		...
		$breadcrumb = '<div id="main_breadcrumb" class="breadcrumb">';

		/* If $before was set, wrap it in a container. */
		$breadcrumb .= ( !empty( $args['before'] ) ? '<span class="trail-before">' . $args['before'] . '</span> ' : '' );

		/* Wrap the $trail['trail_end'] value with microformat. */
		if ( !empty( $trail['trail_end'] ) ) {
			$trail['trail_end'] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><span itemprop="title">'.$trail['trail_end'].'</span></div>';
		}
...
function breadcrumb_trail_get_items( $args = array() ) {
	...
	if ( !is_front_page() && $args['show_home'] )
		$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . home_url() . '" itemprop="url" title="' . esc_attr( get_bloginfo( 'name' ) ) . '" rel="home" class="trail-begin"><span itemprop="title">' . $args['show_home'] . '</span></a></div>';
	...
	elseif ( is_singular() ) {
		...
		elseif ( 'page' !== $post_type ) {
			...
			if ( !empty( $post_type_object->has_archive ) )
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_post_type_archive_link( $post_type ) . '" itemprop="url" title="' . esc_attr( $post_type_object->labels->name ) . '"><span itemprop="title">' . $post_type_object->labels->name . '</span></a></div>';
		}
		...
		if ( !empty( $args["singular_{$post_type}_taxonomy"] ) && $terms = get_the_terms( $post_id, $args["singular_{$post_type}_taxonomy"], '', ', ', '' ) ) {
			foreach($terms as $t)
				$current_term = $t;
			$term_parent = get_term( $current_term->parent, $args["singular_{$post_type}_taxonomy"] );
			if( $term_parent ) {
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_term_link( $term_parent ) . '" itemprop="url" rel="up up" title="' . esc_attr( $term_parent->name ) . '"><span itemprop="title">' . $term_parent->name . '</span></a></div>';
			}
			$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_term_link( $current_term ) . '" itemprop="url" rel="up" title="' . esc_attr( $current_term->name ) . '"><span itemprop="title">' . $current_term->name . '</span></a></div>';
		}

		/* End with the post title. */
		$post_title = get_the_title();
		if ( !empty( $post_title ) )
			$trail['trail_end'] = ode_shorten_text($post_title, 40);
	}
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_year_link( get_the_time( 'Y' ) ) . '" itemprop="url" title="' . get_the_time( esc_attr__( 'Y', $textdomain ) ) . '"><span itemprop="title">' . get_the_time( __( 'Y', $textdomain ) ) . '</span></a></div>';
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" itemprop="url" title="' . get_the_time( esc_attr__( 'F', $textdomain ) ) . '"><span itemprop="title">' . get_the_time( __( 'F', $textdomain ) ) . '</span></a></div>';
	...
	elseif ( is_404() )
		$trail['trail_end'] = 'Page introuvable';
...
function breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '', $args = array() ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag )
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" itemprop="url" title="' . get_the_time( esc_attr__( 'Y', $textdomain ), $post_id ) . '"><span itemprop="title">' . get_the_time( __( 'Y', $textdomain ), $post_id ) . '</span></a></div>';

			/* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag )
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" itemprop="url" title="' . get_the_time( esc_attr__( 'F Y', $textdomain ), $post_id ) . '"><span itemprop="title">' . get_the_time( __( 'F', $textdomain ), $post_id ) . '</span></a></div>';

			/* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag )
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" itemprop="url" title="' . get_the_time( esc_attr__( 'F j, Y', $textdomain ), $post_id ) . '"><span itemprop="title">' . get_the_time( __( 'd', $textdomain ), $post_id ) . '</span></a></div>';

			/* If using the %author% tag, add a link to the post author archive. */
			elseif ( '%author%' == $tag )
				$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_author_posts_url( $post->post_author ) . '" itemprop="url" title="' . esc_attr( get_the_author_meta( 'display_name', $post->post_author ) ) . '"><span itemprop="title">' . get_the_author_meta( 'display_name', $post->post_author ) . '</span></a></div>';
			...
			elseif ( '%category%' == $tag && 'category' !== $args["singular_{$post->post_type}_taxonomy"] ) {
				...
				if ( $terms ) {
					...
					$trail[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_term_link( $term, 'category' ) . '" itemprop="url" title="' . esc_attr( $term->name ) . '"><span itemprop="title">' . $term->name . '</span></a></div>';
				}
			}
		}
	}
...
function breadcrumb_trail_get_parents( $post_id = '', $path = '' ) {
	...
	while ( $post_id ) {
		...
		$parents[]  = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_permalink( $post_id ) . '" itemprop="url" title="' . esc_attr( get_the_title( $post_id ) ) . '"><span itemprop="title">' . get_the_title( $post_id ) . '</span></a></div>';
...
function breadcrumb_trail_get_term_parents( $parent_id = '', $taxonomy = '' ) {
	...
	while ( $parent_id ) {
		...
		$parents[] = '<div itemscope="itemscope" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_term_link( $parent, $taxonomy ) . '" itemprop="url" title="' . esc_attr( $parent->name ) . '"><span itemprop="title">' . $parent->name . '</span></a></div>';
